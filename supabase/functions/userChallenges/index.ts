  ...
  try {
    ...
    const response = await supabaseFetch(
      `${supabaseUrl}/rest/v1/users_challenges`,
      {
        method: "POST",
        body: JSON.stringify({
          user_id: user_id, // Use the resolved user_id
...
//     // Check if user exists based on user_uuid
//     const userResponse = await supabaseFetch(
//       `${supabaseUrl}/rest/v1/users?uuid=eq.${body.user_uuid}`,
//       { method: "GET" }
//     );
...
// PUT: Update user-challenge relationship (completed and favorites)
...
// const createUserChallenge = async (req: Request) => {
//   try {
//     // body is already an object, so no need for req.text() like is seen elsewhere
//     const body = req;
...
// // const corsHeaders = {
// //   "Content-Type": "application/json",
// //   "Access-Control-Allow-Origin": "*", // Replace "*" with frontend's URL once it is deployed
// //   "Access-Control-Allow-Methods": "GET, POST, DELETE, OPTIONS", // Allow these methods for CORS
// //   "Access-Control-Allow-Headers": "Content-Type, Authorization, apikey", // Allow headers for requests
// // };

// // // Helper function that fetches data from Supabase
// // const supabaseFetch = async (url: string, options: RequestInit) => {
// //   const response = await fetch(url, {
// //     ...options,
// //     headers: {
...
// //     const body = await req.json(); // Use req.json() to parse JSON body
